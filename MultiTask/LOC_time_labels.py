...
from MultiTask.utils.utilities import (create_folder, read_audio, calculate_scalar_of_tensor,
...
NUM_DAYS = 2
NUM_WEEKS = 4
space_time_path = '/home/liuzhuangzhuang/pycharm_P/task5-多任务0.0/work_space/features/logmel_64frames_64melbins'
annotation_path = 'C:\Users\AHMED\Desktop\Bach\annotations.csv'
...
def get_subset_split(annotation_data):
    ...
    return np.array(train_idxs), np.array(valid_idxs),np.array(eval_idxs)


annotation_data = pd.read_csv(annotation_path, low_memory=False).sort_values('audio_filename')
...
X_eval_time = np.array([
    one_hot(year_list[idx], NUM_YEARS) \
    + one_hot(week_list[idx] - 1, NUM_WEEKS) \
    + one_hot(day_list[idx], NUM_DAYS) \
    + one_hot(hour_list[idx], NUM_HOURS)
    for idx in eval_idex])
...
X_valid_loc = np.array([[latitude_list[idx],
                             longitude_list[idx]]
                            for idx in valid_idxs])
X_eval_loc = np.array([[latitude_list[idx],
                             longitude_list[idx]]
                            for idx in eval_idex])
...
X_eval_spacetime = np.concatenate((X_eval_time, X_eval_loc), axis=-1)
...
hf3 = h5py.File(os.path.join(space_time_path, 'evalspacetime.h5'), 'w')
